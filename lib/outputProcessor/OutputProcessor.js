...
OutputProcessor.prototype.process = function (contextItem, opts) {
    var me = this;
    ...
    if (!opts.isDebug) {
        outputDir = util.resolvePath(
            opts.output ? opts.output : './analysedResult'
        ) + '/';
        ...
        outputDir = util.resolvePath('./lib/output/src/mockData/') + '/';
        if (fs.existsSync(outputDir)) {
            edp.util.rmdir(outputDir);
        }
        mkdirp.sync(outputDir);
    ...
    if (contextItem.toWriteList) {
        for (var path in contextItem.toWriteList) {
            toWriteList.push({
                outputPath: util.resolvePath(
                    outputDir + (opts.isDebug ? '' : 'asset/mockData/')
                    + path + '.js'
                ),
                data: require('util').format(
                    outputTpl,
                    JSON.stringify(contextItem.toWriteList[path], null, 4)
                )
            });
        }
    }
